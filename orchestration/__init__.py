...
class DagModel(BaseModel):
    name: Dag_Name | str
    description: str
    params: dict[str, Base_Type | list[Base_Type]] | None
...
def default_config_parser(conf: dict[Any, Any]) -> Result[ConfigModel, ConfigParsingFailure]:
    ...
    match conf:
        ...
        case _:
            return Failure("Could not parse config structure {conf} with default config parser.")
...
class QRCP:
    ...
    def __init__(self, conf: Any, parser : Config_Parser = default_config_parser) -> None:
    ...
    def get(self, field: Config_Field_Name) -> Result[ConfigFieldNotFound, str | list[str] | dict[Any, Any]]:
    ...
    def get_dag_params(self: QRCP, dag: Data_Source | str) -> Result[dict[str, Base_Type | list[Base_Type]], GentropyConfigNotFound]:
        """Get gentropy dag parameters from existing configuration.
        Returns:
            Result[dict[str, Base_Type | list[Base_Type]], GentropyConfigNotFound]: Result of Gentropy dag params lookup.
        """
        for d in self.config.DAGS:
            if d.name == dag and d.params:
                return Success(d.params)
        return Failure("Gentropy Config was not found")
