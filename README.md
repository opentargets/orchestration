...
- [Terraform](https://www.terraform.io/downloads.html) (optional, for cloud
   instance setup)
...
> [!WARNING]
> On macOS if you already have vscode installed, make sure to enable the terminal
> shortcut see [thread](https://code.visualstudio.com/docs/setup/mac#_manually-configure-the-path)
...
Local development sets up the Airflow service using Docker Compose with
LocalExecutor. Local service is exposed to the `localhost:8080` by default.

> [!NOTE]
> The directory containing the orchestration code is mounted into the Airflow
service containers, so you can edit the code locally in your IDE and see the
changes reflected in the Airflow UI.
...
In order to use the local Airflow instance yoy need to have the google cloud
...
Make sure that you have enabled the application default credentials to run the
terraform.
...
Google Cloud; and open vscode into that instance code as well as a the Airflow
UI in a browser automatically. The default credentials are `airflow`/`airflow`.
...
Besides running the whole pipeline, it is possible to run single steps for
development or testing purposes. You are encouraged to do this in a local env.

To enable a dev run, edit `src/orchestration/dags/config/unified_pipeline.yaml`:

```yaml
release_name: 25.06
is_dev: true
dev_run_name: jferrer/25.06-testrun-my-biosample
dev_run_step: gentropy_biosample
```

Setting the above config will run _only_ the `gentropy_biosample` step, fetching
any needed input from the `25.06` release, and writing the outputs into
`jferrer/25.06-testrun-my-biosample`.

> [!WARNING]
> The step will run **unconditionally**. No checks will be made and data in that
> folder will be overwritten.

> [!NOTE]
> As soon as the step is sent to run, i.e. the airflow task `run_{step_name}` is
> started (light green border) and the dataproc job status is `started`, you can
> shut down the orchestrator. The step will still run and the cluster will be
> decomissioned after.
